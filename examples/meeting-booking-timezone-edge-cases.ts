...
function createTimezoneEdgeCaseOrg(): Organization {
  return {
...
function createTimezoneEdgeCaseUsers(): User[] {
  return [
    {
      id: 'alice-tz',
...
const timezoneEdgeCaseAction = {
  ...
  async execute(httpClient: SimulatedHTTPClient) {
    ...
    const freeBusyResponse = await httpClient.post(
    ...
    return {
      success: true,
      meeting: {
        id: (createResponse.body as any).id,
 ...
 */
function createTimezoneEdgeCaseRubric(): EvaluationRubric {
...
export async function runTimezoneEdgeCaseScenario() {
  ...
  if (data?.edgeCases) {
    ...
    data.edgeCases.forEach((edgeCase: string) => {
  ...
  console.log(
    `  Date Boundary Handling: ${score.byTarget['Date Boundary Handling'].toFixed(1)}%`
  );
  console.log(
    `  Timezone Conversion Accuracy: ${score.byTarget['Timezone Conversion Accuracy'].toFixed(1)}%`
  );
  console.log(
    `  Edge Case Documentation: ${score.byTarget['Edge Case Documentation'].toFixed(1)}%`
  );
  console.log(
    `  Working Hours Respect: ${score.byTarget['Working Hours Respect'].toFixed(1)}%`
  );
...
// Export for testing
export {
  timezoneEdgeCaseAction,
  createTimezoneEdgeCaseRubric,
  createTimezoneEdgeCaseUsers,
  createTimezoneEdgeCaseOrg,
};
