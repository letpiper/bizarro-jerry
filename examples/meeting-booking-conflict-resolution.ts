...
function createConflictResolutionOrg(): Organization {
  return {
...
function createConflictResolutionUsers(): User[] {
  return [
    {
      id: 'alice-conflict',
...
const conflictResolutionAction = {
  ...
  async execute(httpClient: SimulatedHTTPClient) {
    ...
    const calendars = (freeBusyResponse.body as any).calendars;
    ...
    if (aliceBusy && bobFree && charlieFree) {
      ...
      const backupFreeBusyResponse = await httpClient.post(
        'https://www.googleapis.com/calendar/v3/freeBusy',
        {
 ...
 */
function createConflictResolutionRubric(): EvaluationRubric {
    ...
    criteria: [
      {
        ...
        evaluate: (result) => {
          if (!result.success) return 0;
          const data = result.result as any;
          if (!data?.originalTime || !data?.finalTime) return 0;
          ...
          if (diffHours >= 1 && diffHours <= 2) return 100;
...
export async function runConflictResolutionScenario() {
  ...
  console.log(
    `  Alternative Finding: ${score.byTarget['Alternative Finding'].toFixed(1)}%`
  );
...
export {
  ...
  createConflictResolutionRubric,
  createConflictResolutionUsers,
  createConflictResolutionOrg,
};
