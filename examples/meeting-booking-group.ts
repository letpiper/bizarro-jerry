...
function createGroupMeetingOrg(): Organization {
  return {
...
function createGroupMeetingUsers(): User[] {
  return [
    {
      id: 'alice-group',
...
const groupMeetingAction = {
  ...
  async execute(httpClient: SimulatedHTTPClient) {
    ...
    const createEvent = {
      ...
      description:
        'Quarterly sync across US, UK, and India offices. Note: Not ideal for all timezones.',
 ...
 */
function createGroupMeetingRubric(): EvaluationRubric {
...
export async function runGroupMeetingScenario() {
  ...
  console.log(
    `  Multi-timezone Coordination: ${score.byTarget['Multi-timezone Coordination'].toFixed(1)}%`
  );
  ...
  console.log(
    `  Alternative Suggestions: ${score.byTarget['Alternative Suggestions'].toFixed(1)}%`
  );
...
export {
  groupMeetingAction,
  createGroupMeetingRubric,
  createGroupMeetingUsers,
  createGroupMeetingOrg,
};
